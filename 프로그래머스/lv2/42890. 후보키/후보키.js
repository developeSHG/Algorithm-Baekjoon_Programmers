...
const minimality = (answer, elements) => {
    return answer.length
        ? answer.every(
              (arr) =>
                  arr.filter((a) => elements.some((b) => a === b)).length !==
                  arr.length
          )
        : true;
};
...
function solution(relation) {
    const answer = [];
    const [attribute, row, column] = [
        Array.from({ length: relation[0].length }, (_, idx) => idx),
        relation.length,
        relation[0].length,
    ];
    ...
    for (let i = 1; i <= column; ++i) {
        ...
        combinations.forEach((elements) => {
            if (
                uniqueness(relation, elements, row) &&
                minimality(answer, elements)
            ) {
